...
def _get_conn():
    try:
        ...
        return psycopg2.connect(
            ...
            dbname=os.environ.get("POSTGRES_DB", "interview_coach_mcq"),
        ...
        with conn:
            with conn.cursor() as cur:
                ...
                CREATE TABLE IF NOT EXISTS mcq_responses (
                    ...
                    session_id       UUID NOT NULL REFERENCES mcq_sessions(session_id),
                    question_id      VARCHAR(50) NOT NULL REFERENCES mcq_questions(question_id),
                    skill            VARCHAR(100) NOT NULL,
                ...
                CREATE TABLE IF NOT EXISTS mcq_skill_profiles (
                    ...
                    proficiency_label VARCHAR(20) NOT NULL,
                ...
                CREATE INDEX IF NOT EXISTS idx_mcq_q_skill     ON mcq_questions(skill);
                CREATE INDEX IF NOT EXISTS idx_mcq_q_b         ON mcq_questions(b_param);
                CREATE INDEX IF NOT EXISTS idx_mcq_resp_sess   ON mcq_responses(session_id);
                CREATE INDEX IF NOT EXISTS idx_mcq_resp_skill  ON mcq_responses(skill);
                CREATE INDEX IF NOT EXISTS idx_mcq_prof_skill  ON mcq_skill_profiles(skill);
                CREATE INDEX IF NOT EXISTS idx_mcq_prof_theta  ON mcq_skill_profiles(theta_final DESC);
                """)
        print("[MCQ DB] Schema initialised.")
        return True
    except Exception as e:
...
def complete_session(session_id: str, theta_overall: float,
    ...
    """Mark session as completed and store final overall θ."""
    conn = _get_conn()
    if not conn:
        return
    try:
