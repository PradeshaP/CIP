...
# ── Constants (from senior's CFG dict) ───────────────────────────────────────
...
class SkillSession:
    """
    Tracks one student's adaptive quiz for ONE skill.
    Mirrors senior's StudentSession class (Cell 3) but:
      - Works with dict-based questions (from DB) not DataFrame rows
      - b updates written back here and persisted to DB separately
      - Designed to be serialisable into st.session_state

    Key behaviours from senior's Cell 3:
      ✅ θ updates after every answer
      ✅ b_online updates per response
      ✅ batch b calibration after CALIB_MIN_RESPONSES
      ✅ SE(θ) tracked via Fisher information
      ✅ last_correct tracked for step-down selection
    """
